...
void LIGHT_CONTROLS::flash_lights(bool dipped, bool fog, uint8_t duration_msec) {
    ...
    x.data[1] = duration_msec;
    DPRINTLN(F("DIPPED BEAM ON"));
    canbus->sendToBus(&x);
}

void LIGHT_CONTROLS::flash_indicator(INDICATOR i, uint8_t duration_msec) {
    x.can_id = 0x000E;
    x.can_dlc = 2;
    x.data[0] = i;
    x.data[1] = duration_msec;
...
void LIGHT_CONTROLS::flash_hazard(uint8_t duration_msec) {
    x.can_id = 0x000E;
    x.can_dlc = 2;
    x.data[0] = 0xE0;
    x.data[1] = duration_msec;
